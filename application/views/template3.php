        ...
        <div class="wrapper">
            ...
            <header class="main-header">
                ...
                <a href="<?php echo site_url('dashboard') ?>" class="logo">
                    ...
                    <span class="logo-mini"><b><?= $infoweb->nama_web?></b></span>
                    <!-- logo for regular state and mobile devices -->
                    <span class="logo-lg"><b><?= $infoweb->nama_web?></b></span>
                </a>
                        ...
                        <ul class="nav navbar-nav">
                            ...
                            <li class="dropdown user user-menu">
                                <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                    <img src="<?php echo base_url()?>template/dist/img/<?=$this->session->userdata("fot")?>" class="user-image" alt="User Image">
                                    <span class="hidden-xs"> <?php echo $this->session->userdata("first_name"); ?></span>
                                </a>
                                <ul class="dropdown-menu">
                                    <!-- User image -->
                                    <li class="user-header">
                                        <img src="<?php echo base_url()?>template/dist/img/<?=$this->session->userdata("fot")?>" class="img-circle" alt="User Image">
                                        <p>
                                    ...
                                    <li class="user-footer">
                                        <div class="pull-left">
                                            <a href="<?= site_url()?>users/read/<?= $this->session->userdata("user_id")?>" class="btn btn-default btn-flat">Profile</a>
                                        </div>
            ...
            <aside class="main-sidebar">
                ...
                <section class="sidebar">
                    ...
                    <ul class="sidebar-menu">
                        ...
                        $menu = $this->db->get_where('view_akses', array('is_parent' => 0,'is_active'=>1,'username'=>$this->session->userdata('user_id')));
                        foreach ($menu->result() as $m) {
                            $menuaktif = ($uris==$m->link) ? "active" : '' ;
                            $menuktif = ($menus->is_parent==$m->id) ? "active" : '' ;
                            // chek ada sub menu
                            $submenu = $this->db->get_where('menu',array('is_parent'=>$m->id_child,'is_active'=>1));
                            if($submenu->num_rows()>0){
                                // tampilkan submenu
                                echo "<li class='".$menuktif." treeview'>
                                    ".anchor('#',  "<i class='$m->icon'></i>".strtoupper($m->name).' <i class="fa fa-angle-left pull-right"></i>')."
                                        <ul class='treeview-menu'>";
                                foreach ($submenu->result() as $s){
                                    $subaktif = ($uris==$s->link) ? "active" : '' ;
                                    echo "<li class='".$subaktif."'>" . anchor($s->link, "<i class='$s->icon'></i> <span>" . strtoupper($s->name)) . "</span></li>";
                                }
                                   echo"</ul>
                                    </li>";
                            }else{
                                echo "<li class='".$menuaktif."'>" . anchor($m->link, "<i class='$m->icon'></i> <span>" . strtoupper($m->name)) . "</span></li>";
                            }
